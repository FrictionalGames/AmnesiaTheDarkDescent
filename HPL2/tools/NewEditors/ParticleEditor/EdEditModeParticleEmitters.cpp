...
void cEdEditModeParticleEmitters::SetSelectedEmitter(iEdObject* apEmitter)
{
	...
	Log("Selecting emitter %p\n", apEmitter);

	if(mpSelectedEmitterPane)
	{
		mpSelectedEmitterPane->SetActive(false);
...
bool cEdEditModeParticleEmitters::SetUpCreationData(iEdObjectData* apData)
{
	return true;
}
...
void cEdEditModeParticleEmitters::OnWorldLoad()
{
	const tEdObjectMap& mapObjects = GetWorld()->GetObjects();
	iEdObject* pFirstEmitter = mapObjects.begin()->second;

	SetSelectedEmitter(pFirstEmitter);
}
